...
填写好项目配置信息后，就可以开始打包项目了：按照下图所示的步骤依次执行，最后打包好的 **`ROOT.war`** 将会存放在 **`target`**
目录中。

### 安装Docker

docker的安装流程可参考下面这篇文章，笔者在这里就不再赘述了：*
...
1：执行如下命令拉取Tomcat的Docker镜像（注意：需要`8.5`版本）
...
5：成功运行后，访问配置文件中填写的服务器ip地址即可 http://127.0.0.1/login